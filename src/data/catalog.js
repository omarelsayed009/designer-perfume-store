...
export const MEN_LIMIT = 16;
export const WOMEN_LIMIT = 10;
...
export async function loadCatalog() {
  try {
    ...
    const sourceProducts = remoteProducts.length ? [...remoteProducts, ...localCatalog] : [...fallbackProducts, ...localCatalog];

    return sourceProducts
      .map((product, index) => normalizeProduct(product, index))
      .filter((product, index, array) => array.findIndex((entry) => entry.id === product.id) === index);
  } catch {
    return [...fallbackProducts, ...localCatalog]
      .map((product, index) => normalizeProduct(product, index))
      .filter((product, index, array) => array.findIndex((entry) => entry.id === product.id) === index);
  }
}
